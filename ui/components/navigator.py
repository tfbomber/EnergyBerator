...
def render_navigator(navigate_to):
    ...
    st.button("🏘 PV Opportunity Street Ranking",
              use_container_width=True,
    ...
    st.markdown("---")
    st.markdown("**输出模块**")
    st.button("⏳ 合规时间线 (Timeline)", 
              use_container_width=True,
              on_click=set_view_and_nav, args=("PROPERTY", navigate_to, None))
              
    st.button("📊 报告与导出 (Report)", 
              use_container_width=True,
    ...
    st.markdown("---")
    st.markdown("**市级洞察 (City Insights)**")
    st.button("📍 诺伊斯 MVP (Neuss Target)", 
              use_container_width=True, 
    ...
    st.button("🏗️ Foundation Filter (结构筛查)", 
              use_container_width=True, 
